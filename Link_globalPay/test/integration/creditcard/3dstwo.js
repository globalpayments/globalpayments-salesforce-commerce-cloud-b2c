...
describe('billingForm', function () {
    ...
    it('positive test for 3ds2 card', function () {
        ...
        return request(myRequest)
            .then(function (addToCartResponse) {
                ...
                assert.equal(versionCheckData.statusCode, 200, 'Expected authentication response to be 200.');
                cookieString = cookieJar.getCookieString(myRequest.url);
                myRequest.url = config.baseUrl + '/GlobalPay-Initiation';
                var cookie = request.cookie(cookieString);
                myRequest.body = {
                    serverTransactionId: versionCheckData.serverTransactionId,
                    challengeNotificationUrl: '',
                    authId: 'AUT_JRjQYgST3jLw2rpx65hqofAPowCw5d',
                    ...
                    card: {
                        reference: 'PMT_bbc1f9bc-b71f-47f0-92ba-ff72860e57a5',// resp.paymentReference,
                        cartData: {
                            amount: 3000, address1: "182 Eagle Street", city: "Fairview Heights", postalcode: "62208"
                        }
                    }
                };
                myRequest.json = true;
                cookieJar.setCookie(cookie, myRequest.body);
                cookieJar.setCookie(cookie, myRequest.url);
                //step3 : get cookies, make initialize call, then set cookies
                return request(myRequest);

            }).then(function (initResponse) {
                assert.equal(initResponse.statusCode, 200, 'Expected initiation request statusCode to be 200.');
                cookieString = cookieJar.getCookieString(myRequest.url);
