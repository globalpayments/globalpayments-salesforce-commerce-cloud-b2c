...
server.get('GetCurrencies', function (req, res, next) {
    var currencies = ['INR', 'USD', 'EUR'];
    var currencies = [];
    req.querystring.currencies.split(',').forEach(function (eachccy) {
        currencies.push(eachccy);
    });
    res.json({
        success: true,
        currencies: currencies// [req.querystring.currencies]
    });
    next();
});
/**
